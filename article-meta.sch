...
<pattern id="article_meta" xmlns="http://purl.oclc.org/dsdl/schematron">
	<rule context="article-meta">
		...
		<assert test="matches(./volume,$yearVal)" role="error" diagnostics="volume-incorrect"/>
		...
		<assert test="related-article[@ext-link-type='doi']" role="error" diagnostics="invalid-relart-extlink-doi"/>
		<assert test="matches(related-article/@related-article-type, 'addendum|commentary|commentary-article|companion|corrected-article|correction-forward|in-this-issue|letter|partial-retraction|retracted-article|retraction-forward')" role="error" diagnostics="invalid-relart-addendum"/>
		<assert test="matches(related-article/@xlink:href,$doi)" role="error" diagnostics="invalid-relart-href-doi"/>
	</rule>
	...
	<diagnostics>
		...
		<diagnostic id="volume-incorrect">Given volume <value-of select="./volume"/> does not match with publication year <value-of select="$year"/>.</diagnostic>
		...
		<diagnostic id="invalid-relart-href-doi">Incorrect related-article href <value-of select="related-article/@xlink:href"/>. Expecting <value-of select="$doi"/></diagnostic>
		<diagnostic id="invalid-relart-extlink-doi">Invalid ext-link-type appear in xml file <value-of select="ext-link"/>It should be 'doi' format only.</diagnostic>
		<diagnostic id="invalid-relart-addendum">Invalid related-article-type appear in xml file.<value-of select="related-article"/> </diagnostic>
	</diagnostics> 
</pattern>